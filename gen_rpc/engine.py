...
Info
    Engine orchestrating the initialization and execution of gen_rpc.
'''
...
class GenRPC(Base):
    '''
        ...
        It defines:
            ...
            :attributes:
                | _is_initialized - The flag indicating whether the gen_rpc engine is initialized.
            ...
            :methods:
                | __init__ - Initializes the gen_rpc engine with adapters and services.
                | process - Processes the gen_rpc commands.
    '''
    ...
    def __init__(self, bundle: GenRPCBundle) -> None:
        '''
            Initializes the gen_rpc engine with adapters and services.

            :param bundle: gen_rpc bundle containing adapters and services.
            :exceptions: None.
        '''
        ...
        try:
            ...
            self._logger.write_log(INFO, '✅ gen_rpc: engine initialized successfully!')

        except (ATSValueError, ATSTypeError) as exc:
            stdout.write(f'❌ gen_rpc: {exc}!\n')

        except Exception as exc:
            stdout.write(f'❌ gen_rpc unexpected exception: {exc}!\n')

    def process(self) -> bool:
        '''
            Processes the gen_rpc commands.
        ...
        try:
            if self.is_initialized():
                ...
                if result.get("returncode") != 0:
                    self._logger.write_log(ERROR, f'❌ gen_rpc: {result.get("stderr") or "failed!"}')
                    return False
                else:
                    self._logger.write_log(INFO, '✅ gen_rpc: done!')
                    self._logger.write_log(INFO, '✅ gen_rpc: exiting successfully!')
                    return True
            else:
                self._logger.write_log(ERROR, '❌ gen_rpc: engine not initialized!')
                return False

        except (ATSValueError, ATSTypeError) as exc:
            self._logger.write_log(ERROR, f'❌ gen_rpc: {exc}!')
            return False

        except Exception as exc:
            self._logger.write_log(ERROR, f'❌ gen_rpc unexpected exception: {exc}!')
            return False
