...
class GenRPC(CfgCLI):
    ...
    def __init__(self, verbose=False):
        ...
        gen_rpc_property = {
            ...
            'ats_repository': 'gen_rpc',
            'ats_name': 'gen_rpc',
            'ats_logo_path': '{0}{1}'.format(current_dir, GenRPC.LOGO),
            'ats_use_github_infrastructure': True
        }
        splash = Splash(gen_rpc_property, verbose=verbose)
        base_info = '{0}{1}'.format(current_dir, GenRPC.CONFIG)
        CfgCLI.__init__(self, base_info, verbose=verbose)
        verbose_message(GenRPC.GEN_VERBOSE, verbose, 'init tool info')
        self.logger = ATSLogger(
            GenRPC.GEN_VERBOSE.lower(),
            '{0}{1}'.format(current_dir, GenRPC.LOG),
            verbose=verbose
        )
        if self.tool_operational:
            self.add_new_option(
                GenRPC.OPS[0], GenRPC.OPS[1],
                dest='gen', help='generate RPC modules'
            )
            self.add_new_option(
                GenRPC.OPS[2], GenRPC.OPS[3],
                action='store_true', default=False,
                help='activate verbose mode for generation'
            )
            self.add_new_option(
                GenRPC.OPS[4], action='version', version=__version__
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_operational:
            ...
            if num_of_args_sys > 1:
                ...
                if operation not in GenRPC.OPS:
                    sys.argv.append('-h')
            else:
                sys.argv.append('-h')
            ...
            if not pro_exists:
                if bool(getattr(args, 'gen')):
                    print(
                        '{0} {1} [{2}]'.format(
                            '[{0}]'.format(GenRPC.GEN_VERBOSE.lower()),
                            'generating', getattr(args, 'gen')
                        )
                    )
                    generator = RPCService(
                        getattr(args, 'gen'), verbose=verbose
                    )
                    status = generator.gen_setup(verbose=verbose)
                    if status:
                        success_message(GenRPC.GEN_VERBOSE, 'done\n')
                        self.logger.write_log(
                            '{0} {1} done'.format(
                                'generating RPC modules',
                                getattr(args, 'gen')
                            ), ATSLogger.ATS_INFO
                        )
                    else:
                        error_message(
                            GenRPC.GEN_VERBOSE, 'generation failed'
                        )
                        self.logger.write_log(
                    ...
                    error_message(
                        GenRPC.GEN_VERBOSE, 'provide project name'
                    )
                    self.logger.write_log(
                ...
                error_message(
                    GenRPC.GEN_VERBOSE, 'project already exist'
                )
                self.logger.write_log(
            ...
            error_message(
                GenRPC.GEN_VERBOSE, 'tool is not operational'
            )
            self.logger.write_log(
